-- create_invitation + accept_invitation RPCs.
--
-- Move the cap-check / dup-check / hash / accept+link workflow into the
-- database, where they run inside a single transaction with row-level
-- locks. Closes the TOCTOU windows that exist when the same logic is
-- split across multiple PostgREST round-trips.
--
-- Code-hashing rule (must match src/lib/invite-codes.ts):
--   normalize(code) = uppercase, A-Z and 2-9 only, exactly 6 chars
--   hash = sha256(invite.id::text || ':' || normalize(code))  → hex
-- Pure SHA-256, no per-row salt other than the row UUID itself. Hash
-- stored in code_hash; raw code is never persisted.
...
-- create_invitation
-- ─────────────────────────────────────────────────────────────────
-- Caller generates the raw 6-char code (so it can show it once and
-- compose the email) and passes it here. RPC generates the row UUID
-- upfront, hashes (id || ':' || code) inline, and inserts in one shot —
-- no rotate-after-insert dance, no torn write to recover from.
--
-- Returns a JSONB envelope:
--   { ok: true,  invite: {id, token, patient_email, expires_at},
--     nutri_name }
--   { ok: false, code: 'self_invite' | 'hourly_cap' | 'daily_cap'
--                       | 'duplicate_pending' | 'invalid_email',
--     error: '...' }
-- Raises only for unauthorized / forbidden (clean errors for the route).
...
begin
  ...
  -- Cap checks (in-tx so a parallel insert in flight is double-counted).
  ...
  -- Dup pre-check (the partial unique index from migration 028 is the
  -- actual race-proof guarantee — the 23505 catch below handles the loss).
  ...
  computed_hash := encode(digest(new_id::text || ':' || normalized_code, 'sha256'), 'hex');
...
-- accept_invitation
-- ─────────────────────────────────────────────────────────────────
-- Atomic: SELECT ... FOR UPDATE on the invite row, validate, optionally
-- bump code_attempts on wrong code, lock at 5, upsert the link, mark
-- invite accepted, audit. All one transaction.
...
begin
  ...
  if inv.code_hash is not null then
    ...
    actual_hash := encode(digest(inv.id::text || ':' || normalized_code, 'sha256'), 'hex');
